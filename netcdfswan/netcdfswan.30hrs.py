...
class NetCDFSWAN(S3NetCDF):
    ...
    def loadSpc(filepath, return_metadata=False, monthOnly=None):
        ...
        :type filepath: str
        :param return_meta: return metadata only
        ...
        while groups:
            ...
            for key in _sub:  # Loop for each variable(e.g Windv_x and Windv_y)
                ...
                if groupName == "s":
                    ...
                    self[groupName, name, _sIndex:_eIndex, sIndex:eIndex] = (
                        array  # Upload
                    )
    ...
    def _uploadPartition(self, groupName, vname):
        ...
        while groups:
            ...
            nodeSlice = slice(startIndex, np.minimum(nnode,
                                                     startIndex + gNode))
            ngNode = nodeSlice.stop - nodeSlice.start
            if self.logger is not None:
                self.logger.info("Uploading group {}".format(fileId))

            filename = os.path.join(self.cacheLocation,
                                    "{}.{}.dat".format(vname, fileId))
            ...
            def work(file):
                ...
                for key in _sub:
                    ...
                    if key_num <= npart:  # in case only first few partitions needed
                        array = _sub[key]
                        fp[:, key_num - 1, sIndex:eIndex] = array[:,
                                                                  nodeSlice].T
