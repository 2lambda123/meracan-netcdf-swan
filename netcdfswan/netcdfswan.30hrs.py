...
class NetCDFSWAN(S3NetCDF):
    ...
    def __enter__(self):
        ...
        self.datetime = startDate + \
            np.arange(ntime)*np.timedelta64(timeStep, 'h')
        ...
        self.ptList = [v["fileKey"]
                       for k, v in self.mvariables.items() if k in pt_gvar] + pt_intr
        ...
        if not os.path.isfile(errorlistPath):
            json.dump([], open(errorlistPath, "w+"))
    ...
    def matFiles(self):
        ...
        if swanFolder is None:
            raise Exception("swanFolder was not specified")
    ...
    def prepareInputJSON(file, swanFolder, **kwargs):
        ...
        if obj['nca']['dimensions']['nsnode'] != meta['nsnode']:
            raise Exception("Please check nsnode in json file. {} to {}".format(
                obj['nca']['dimensions']['nsnode'], meta['nsnode']))

        if obj['nca']['dimensions']['nstation'] != meta['nstation']:
            raise Exception("Please check nstation in json file. {} to {}".format(
    ...
    def printMatKeys(swanFolder, year=2014):
        ...
        files = NetCDFSWAN.getFiles(swanFolder)
        files = list(filter(
            lambda file: "year" in file and file["year"] == year and "month" in file and file["month"] == 1 and file['ext'] == '.mat', files))
        for file in files:
    ...
    def printSpcShape(swanFolder, year=2014):
        ...
        files = NetCDFSWAN.getFiles(swanFolder)
        files = list(filter(
            lambda file: "year" in file and file["year"] == year and "month" in file and file["month"] == 1 and file['ext'] == '.spc', files))
        for file in files:
            print(
                "{} - {}".format(file["name"], NetCDFSWAN.load(file["path"])['spectra'].shape))
    ...
    def getSpectralStationMetadata(swanFolder, print_meta=False, year=2014, month=1):
        ...
        files = list(filter(lambda file: "year" in file and "month" in file and file[
                     "year"] == year and file["month"] == month and file['ext'] == '.spc', files))

        files = sorted(files, key=lambda x: x["name"])
        stations = {}
        isnode = 0
        for i, file in enumerate(files):
            nlatlng = NetCDFSWAN.load(
                file['path'], return_metadata=True)['nlatlng']
            stations[file['name']] = {
                "start": isnode, "end": isnode+nlatlng, "id": i}
            isnode += nlatlng
        ...
        for r, d, f in os.walk(swanFolder):
            for file in f:
                ...
                if (folder == "results"):
                    ...
                    files.append({"year": year, "month": month, "path": os.path.join(
                        r, file), "name": name, "ext": ext})
                else:
                    name, ext = os.path.splitext(file)
    ...
    def load(filepath, *args, **kwargs):
        ...
        if ext == ".bot":
            return NetCDFSWAN.loadBot(filepath)
        elif ext == ".ele":
            return NetCDFSWAN.loadEle(filepath)
        elif ext == ".node":
            return NetCDFSWAN.loadNode(filepath)
        elif ext == ".mat":
            return NetCDFSWAN.loadMat(filepath)
        elif ext == ".json":
            return NetCDFSWAN.loadJSON(filepath)
        elif ext == ".spc":
    ...
    def loadSpc(filepath, return_metadata=False, monthOnly=None):
        ...
        with open(filepath, "r") as s:
            ...
            if return_metadata:
                return {"nlatlng": nlonlat, "nfreq": nfreq, "ndir": ndir, "latlng": lonlats, "freq": freq, "dir": dir}
            ...
            while data:
                ...
                if re.match(re_spcdate, token):
                    ...
                    for i in range(nlonlat):
                        ...
                        factor = s.readline().split()[0].strip()
                        factor = float(factor)
                        array[i] = np.array([np.array(
                            [float(int(d)*factor) for d in s.readline().split()]) for afreq in range(nfreq)])
    ...
    def uploadStatic(self, year=2004, month=1):
        ...
        spcFiles = list(filter(lambda file: "year" in file and "month" in file and file[
                        "year"] == year and file["month"] == month and file['ext'] == '.spc', _files))
        ...
        if len(_bot) > 0:
            self['nodes', 'bed'] = NetCDFSWAN.load(_bot[0]['path'])
            update("nodes")
        if len(_ele) > 0:
            self['elem', 'elem'] = NetCDFSWAN.load(_ele[0]['path'])
            update("elem")
        ...
        for spcFile in spcFiles:
            ...
            self['snodes', 'stationid', iIndex:eIndex] = np.zeros(
                eIndex-iIndex, dtype="int32")+id
            update1(name)
        update('spc')

        if pbar:
            pbar.reset(total=len(self.stations.keys()))
    ...
    def loadRemainingFilestoUpload(self, groupName, partName=""):
        ...
        if groupName == "spc":
            ...
            uploadFiles = list(filter(
                lambda name: name != "spectra" and name not in self.ptList, list(self.variables.keys())))
    ...
    def removeUploadedFile(self, groupName, groups, partName=""):
        ...
        if not isinstance(groups, list):
            groups = [groups]  # Make sure it's a list
        path = os.path.join(self.cacheLocation, self.name,
                            "{}{}.json".format(groupName, partName))
        with open(path, "w") as f:
            json.dump(groups, f)
    ...
    def _uploadSpatial(self, groupName):
        ...
        pbar0 = self.pbar0
        if pbar0 is not None:
            pbar0.reset(total=len(groups))

        while groups:
            file = groups.pop(0)
            if pbar0:
                pbar0.set_description(file['name'])
            if self.logger:
                self.logger.info("Uploading {}".format(file['path']))

            try:
                # Load matlab or spc file
                _sub = NetCDFSWAN.load(file['path'], monthOnly=file['month'])
            ...
            sIndex, eIndex = NetCDFSWAN.getDatetimeIndex(
                self.datetime, dt)  # Get datetime index

            for key in _sub:  # Loop for each variable(e.g Windv_x and Windv_y)
                array = _sub[key]
                # Convert matlab/spc name variable to nca name variable
                name = mtname[key]
                if groupName == "s":
                    ...
                    # Spectral file
                    # Need to swap first and second axes
                    array = np.einsum("abcd->bacd", array)
                    _sIndex = stations[file['name']]['start']
                    _eIndex = stations[file['name']]['end']
                    self[groupName, name, _sIndex:_eIndex,
                         sIndex:eIndex] = array  # Upload
    ...
    def _uploadTemporal(self, groupName):
        ...
        while groups:
            vname = groups.pop(0)
            if pbar0:
                pbar0.set_description(vname)
            if self.logger is not None:
                self.logger.info("Uploading group {}".format(vname))
    ...
    def _uploadPartition(self, groupName, vname):
        ...
        while groups:
            ...
            def work(file):
                fp = np.memmap(filename, dtype='float32',
                               mode='r+', shape=(ngNode, npart, ntime))
                _sub = NetCDFSWAN.load(file['path'])
                dt = _sub.pop("datetime")
    ...
    def _uploadPartitionStep2(self, groupName, vname):
        ...
        while groups:
            ...
            if pbar is not None:
                pbar.reset(total=int(ngNode/gnode))
