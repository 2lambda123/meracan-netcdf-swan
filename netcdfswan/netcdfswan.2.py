...
class NetCDFSWAN(S3NetCDF):
    ...
    def __enter__(self):
        ...
        pt_intr = list(
            set(dict(mvariables.items()).keys()).intersection(pt_gvar))
        self.ptList = [v["fileKey"]
                       for k, v in mvariables.items() if k in pt_gvar] + pt_intr
    ...
    def loadRemainingFilestoUpload(self, groupName):
        ...
        if groupName == "spc":
            ...
            uploadFiles = list(filter(
                lambda name: name != "spectra" and name not in self.ptList, list(self.mvariables.keys())))
